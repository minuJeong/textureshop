...
class InitializeTest(unittest.TestCase):
    ...
    def test_num(self):
        ...
        assert isinstance(num_a, np.ndarray)
        assert isinstance(num_b, np.ndarray)
        assert isinstance(num_c, np.ndarray)

    def test_init(self):
        print("[+] Testing init")

        num_a = Num().in_node(init, 0.1).out_node()
        assert num_a.shape == (1, 1, 4)

        num_b = Num(value=0.1).in_node(init).out_node()
        assert num_b[0, 0, 0] == np.float32(0.1)
        assert num_b[0, 0, 1] == np.float32(0.1)
        assert num_b[0, 0, 2] == np.float32(0.1)
        assert num_b[0, 0, 3] == np.float32(0.1)
...
class OperationTest(unittest.TestCase):
    ...
    def test_add(self):
        ...
        assert isinstance(num_a, np.ndarray)
        assert isinstance(num_b, np.ndarray)
        assert isinstance(add_a, np.ndarray)
        assert isinstance(add_b, np.ndarray)
        assert isinstance(add_c, np.ndarray)
    ...
    def test_mul(self):
        ...
        assert isinstance(num_a, np.ndarray)
        assert isinstance(num_b, np.ndarray)
        assert isinstance(mult, np.ndarray)
    ...
    def test_div(self):
        ...
        assert div_b[0, 0, 0] == np.float32(0.2)
        assert div_b[0, 0, 1] == np.float32(0.2)
        assert div_b[0, 0, 2] == np.float32(0.2)
        assert div_b[0, 0, 3] == np.float32(0.2)

        div_c = Divide().in_node(init, 0.6, 0.2).out_node()
        assert div_c[0, 0, 0] == np.float32(3.0)
        assert div_c[0, 0, 3] == np.float32(3.0)

        assert isinstance(num_a, np.ndarray)
        assert isinstance(num_b, np.ndarray)
        assert isinstance(div_a, np.ndarray)
        assert isinstance(div_b, np.ndarray)
        assert isinstance(div_c, np.ndarray)
    ...
    def test_basic_trigonometries(self):
        ...
        assert abs(sin_a[0, 0, 0] - math.sin(0.00)) < PATIENCE
        assert abs(sin_b[0, 0, 0] - math.sin(0.75)) < PATIENCE
        assert abs(sin_c[0, 0, 0] - math.sin(1.50)) < PATIENCE
        assert abs(sin_d[0, 0, 0] - math.sin(222.0)) < PATIENCE
        ...
        assert abs(cos_a[0, 0, 0] - math.cos(0.00)) < PATIENCE
        assert abs(cos_b[0, 0, 0] - math.cos(0.75)) < PATIENCE
        assert abs(cos_c[0, 0, 0] - math.cos(1.50)) < PATIENCE
        assert abs(cos_d[0, 0, 0] - math.cos(222.0)) < PATIENCE
        ...
        assert abs(tan_a[0, 0, 0] - math.tan(0.00)) < PATIENCE
        assert abs(tan_b[0, 0, 0] - math.tan(0.75)) < PATIENCE
        assert abs(tan_c[0, 0, 0] - math.tan(1.50)) < PATIENCE
        assert abs(tan_d[0, 0, 0] - math.tan(222.0)) < PATIENCE
    ...
    def test_arc_trigonometries(self):
        ...
        assert abs(asin_a[0, 0, 0] - math.asin(1.0)) < PATIENCE
        assert abs(asin_b[0, 0, 0] - math.asin(0.1)) < PATIENCE
        assert abs(asin_c[0, 0, 0] - math.asin(-1.0)) < PATIENCE
        assert abs(asin_d[0, 0, 0] - math.asin(-0.5)) < PATIENCE
        ...
        assert abs(acos_a[0, 0, 0] - math.acos(1.0)) < PATIENCE
        assert abs(acos_b[0, 0, 0] - math.acos(0.1)) < PATIENCE
        assert abs(acos_c[0, 0, 0] - math.acos(-1.0)) < PATIENCE
        assert abs(acos_d[0, 0, 0] - math.acos(-0.5)) < PATIENCE
        ...
        assert abs(atan2_a[0, 0, 0] - math.atan2(1.0, 2.0)) < PATIENCE
        assert abs(atan2_b[0, 0, 0] - math.atan2(0.1, 23.2)) < PATIENCE
        assert abs(atan2_c[0, 0, 0] - math.atan2(10.5, 0.11)) < PATIENCE
        assert abs(atan2_d[0, 0, 0] - math.atan2(-100.0, 0.0)) < PATIENCE
    ...
    def test_hyperbolic_trigonometries(self):
        ...
        assert abs(sinh_a[0, 0, 0] - math.sinh(1.0)) < PATIENCE
        assert abs(sinh_b[0, 0, 0] - math.sinh(0.1)) < PATIENCE
        assert abs(sinh_c[0, 0, 0] - math.sinh(2.0)) < PATIENCE
        ...
        assert abs(cosh_a[0, 0, 0] - math.cosh(1.0)) < PATIENCE
        assert abs(cosh_b[0, 0, 0] - math.cosh(0.1)) < PATIENCE
        assert abs(cosh_c[0, 0, 0] - math.cosh(2.0)) < PATIENCE
        ...
        assert abs(tanh_a[0, 0, 0] - math.tanh(1.0)) < PATIENCE
        assert abs(tanh_b[0, 0, 0] - math.tanh(0.1)) < PATIENCE
        assert abs(tanh_c[0, 0, 0] - math.tanh(2.0)) < PATIENCE
    ...
    def test_power_log(self):
        ...
        assert np.all(np.isclose(pow_a, np.power(2.0, 2.0)))
        assert np.all(np.isclose(pow_b, np.power(1.1, 10.0)))
        assert np.all(np.isclose(pow_c, np.power(2.0, 10.0)))
        assert np.all(np.isclose(pow_d, np.power(5.3, 2.1)))

        log_a = Log_Natural().in_node(init, 12.53).out_node()
        log_b = Log_Natural().in_node(init, 22.23).out_node()

        assert np.all(np.isclose(log_a, np.log(12.53)))
        assert np.all(np.isclose(log_b, np.log(22.23)))

        log2_a = Log_2().in_node(init, 12.53).out_node()
        log2_b = Log_2().in_node(init, 22.23).out_node()

        assert np.all(np.isclose(log2_a, np.log2(12.53)))
        assert np.all(np.isclose(log2_b, np.log2(22.23)))
